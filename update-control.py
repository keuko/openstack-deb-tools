...
for package in reqs:
    ...
    if deb_package2_found == 0:
        deb_package2 = subprocess.check_output(
            "grep '^{}\ ' /usr/share/dh-python/dist/cpython2_fallback | cut -d' ' -f2".format(package), shell=True).decode('utf8').strip()
    else:
    ...
    if deb_package3_found == 0:
        deb_package3 = subprocess.check_output(
            "grep '^{}\ ' /usr/share/dh-python/dist/cpython3_fallback | cut -d' ' -f2".format(package), shell=True).decode('utf8').strip()
    else:
